...
async function checkProfiles (agent, proc, timeout,
  ...
  await new Promise((resolve, reject) => {
    ...
    const timeoutObj = setTimeout(() => {
      console.log(proc)
      childProcess.execSync(`cat /proc/${proc.pid}/status`, {stdio: 'inherit'})
      reject(new Error(`Process timed out at : ${Date.now()}`))
    }, timeout)
    ...
    proc
      .on('error', (err) => {console.log(`Reject: ${err}`); reject(err)})
